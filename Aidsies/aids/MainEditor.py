...
selected_map = input("What map to edit: ")

file = open("map"+selected_map.lower()+ ".txt", "r")
MAX_COINS = int(file.readline())
mapWidth= int(file.readline())
mapHeight = int(file.readline())
map2 = list(file.readline())
file.close()
...
def drawMap():
    global xVisibleTiles, yVisibleTiles, xOffset, yOffset, xMapSize, yMapSize, camx, camy, xGset, yGset, coin_counter, first_run, mFont, scoreTex
        ...
        while x < mapWidth:
            ...
            if ID == '1':
                ...
                if first_run:
                    coin_counter+=1
                    print(coin_counter)
                    scoreTex = mFont.render(str(coin_counter) + "/" + str(MAX_COINS), False, (0, 0, 0))
...
def handleEvents():
    global velx, vely, s, onGround, jump, dead, pos, clip, map2, selected_map, first_run, coin_counter, MAX_COINS
    ...
    for event in pygame.event.get():
        ...
        if event.type==QUIT:
            ...
            if event.key == pygame.K_d:
                ...
                MAX_COINS = input("max coins: ")

            elif event.key == pygame.K_p:
                file = open("map" + selected_map + ".txt", 'w')
                file.write(str(MAX_COINS) + "\n")
                file.write(str(mapWidth) + "\n")
                file.write(str(mapHeight) + "\n")
                for c in map2:
                    file.write(c)
                file.close()
...
def drawPlayer():
    ...
    pygame.draw.rect(win, (255,255,255), (0,0,100,50))
...
def update():
    global xNew, yNew, velx, vely, camx, camy, p1x, p1y, onGround, jump, v, dead, vic, map2, bMap, coins, coin, scoreTex, mFont, j, clip, coin_counter
    xNew += velx*s
    if not clip:
        yNew += vely*s
    #gravity
    ...
    if dead:
        ...
        print("Dead!")
        coins=0
        scoreTex = mFont.render(str(coin_counter), False, (0, 0, 0))
        map2 = bMap[:]
    elif coin:
        scoreTex = mFont.render(str(coin_counter), False, (0, 0, 0))
        coin=False

...
def getTile2(x,y):
    if x >= 0 and x < xMapSize and y >= 0 and y < yMapSize:
        return map1[int(y/tileSize)*mapWidth+int(x/tileSize)]
    else:
        return 0
...
def handleEvents():
    global velx, vely, s, onGround, jump, dead, pos, tileSize, xVisibleTiles, yVisibleTiles, xMapSize, yMapSize
...
def update():
    global xNew, yNew, velx, vely, camx, camy, p1x, p1y, onGround, jump, v, dead, vic, map1, bMap, coins, coin, scoreTex, mFont, j
    xNew += velx*s
    #gravity
    if jump:
        vely = -1
...
def main():
    ...
    block1Tex = pygame.transform.scale(block1Tex, (tileSize*a, tileSize*a))
